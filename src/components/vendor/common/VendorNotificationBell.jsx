...
const VendorNotificationBell = () => {
  ...
  const fetchCount = async () => {
    try {
      const count = await getVendorUnreadCount();
  ...
  const fetchRecent = async () => {
    setLoading(true);
    try {
      const data = await getVendorNotifications({ page: 1, limit: 5 });
  ...
  const handleNotificationClick = async (notif) => {
    if (!notif.isRead) {
      try {
        await markVendorAsRead(notif._id);
    ...
    setIsOpen(false);
    if (notif.actionUrl) {
      const targetUrl = resolveNotificationUrl(notif.actionUrl, true);
      if (targetUrl) {
        navigate(targetUrl);
      }
    }
  ...
  const handleClearAll = async (e) => {
    e.stopPropagation();
    if (!window.confirm("Are you sure you want to clear all vendor notifications?")) return;
    try {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="relative rounded-xl bg-slate-100 p-3 hover:bg-slate-200 text-slate-700 transition focus:outline-none"
        title="Notifications"
      >
        <Bell size={20} />
        {unreadCount > 0 && (
          <span className="absolute top-1.5 right-1.5 flex h-4 min-w-[16px] px-1 items-center justify-center rounded-full bg-rose-600 text-[10px] font-bold text-white shadow-sm ring-2 ring-white">
        ...
        <div className="absolute right-0 mt-3 w-96 max-w-[90vw] rounded-2xl bg-white shadow-2xl ring-1 ring-black/5 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center gap-2">
              <h3 className="font-semibold text-slate-800 text-sm">Vendor Notifications</h3>
              {unreadCount > 0 && (
          ...
          <div className="max-h-96 overflow-y-auto divide-y divide-slate-100">
            {loading ? (
              ...
              <div className="p-8 text-center">
                ...
                <p className="text-xs text-slate-400 mt-0.5">We'll alert you of POs, Inspections, and Ratings</p>
              </div>
            ) : (
              notifications.map((n) => (
                <div
