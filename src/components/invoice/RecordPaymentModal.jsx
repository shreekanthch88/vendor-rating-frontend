import React, { useState, useEffect } from "react";
import { X, CreditCard, AlertCircle, IndianRupee } from "lucide-react";
import { recordInvoicePayment } from "../../services/invoiceService";
import { roundToPaise, formatPaise, formatCurrency } from "../../utils/formatters";

const RecordPaymentModal = ({ isOpen, invoice, onClose, onSuccess }) => {
  const rawOutstanding =
    invoice?.payment?.outstandingAmount !== undefined
      ? invoice.payment.outstandingAmount
      : invoice?.totalAmount || 0;

  const outstanding = roundToPaise(rawOutstanding);

  const [amount, setAmount] = useState(formatPaise(outstanding));
  const [paymentMethod, setPaymentMethod] = useState("NEFT");
  const [paymentReference, setPaymentReference] = useState("");
  const [paymentDate, setPaymentDate] = useState(
    new Date().toISOString().split("T")[0]
  );
  const [paymentRemarks, setPaymentRemarks] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  useEffect(() => {
    if (invoice) {
      const curOutstanding = roundToPaise(
        invoice.payment?.outstandingAmount !== undefined
          ? invoice.payment.outstandingAmount
          : invoice.totalAmount || 0
      );
      setAmount(formatPaise(curOutstanding));
    }
  }, [invoice]);

  if (!isOpen || !invoice) return null;

  const numAmount = roundToPaise(amount);
  const remainingAfterPayment = Math.max(0, roundToPaise(outstanding - numAmount));

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!numAmount || numAmount <= 0) {
      setError("Please enter a valid payment amount greater than 0.");
      return;
    }

    if (numAmount > outstanding + 0.01) {
      setError(`Payment amount cannot exceed outstanding balance (${formatCurrency(outstanding)}).`);
      return;
    }

    try {
      setLoading(true);
      setError("");
      await recordInvoicePayment(invoice._id, {
        amount: numAmount,
        paymentMethod,
        paymentReference: paymentReference.trim(),
        paymentDate,
        paymentRemarks: paymentRemarks.trim(),
      });
      onSuccess();
    } catch (err) {
      console.error("Error recording payment:", err);
      setError(err.response?.data?.message || err.message || "Failed to record payment.");
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b bg-slate-900 px-6 py-4 text-white">
          <div className="flex items-center gap-2">
            <CreditCard size={20} className="text-purple-400" />
            <h3 className="text-base font-bold">Record Invoice Payment</h3>
          </div>
          <button
            onClick={onClose}
            className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-800 hover:text-white transition"
          >
            <X size={18} />
          </button>
        </div>

        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="rounded-xl border border-purple-100 bg-purple-50/50 p-4 text-xs text-purple-900 space-y-1">
            <div className="flex justify-between">
              <span>Invoice: <strong>#{invoice.invoiceNumber}</strong></span>
              <span>Total: <strong>{formatCurrency(invoice.totalAmount)}</strong></span>
            </div>
            <div className="flex justify-between border-t border-purple-200/60 pt-1">
              <span>Vendor: <strong>{invoice.vendor?.vendorName}</strong></span>
              <span>Outstanding: <strong className="text-red-600">{formatCurrency(outstanding)}</strong></span>
            </div>
          </div>

          {error && (
            <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-xs text-red-700 flex items-center gap-2">
              <AlertCircle size={15} />
              {error}
            </div>
          )}

          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">
              Payment Amount (₹) *
            </label>
            <input
              type="number"
              min="0.01"
              max={outstanding}
              step="0.01"
              value={amount}
              onChange={(e) => setAmount(e.target.value)}
              className="w-full rounded-lg border border-gray-300 p-2.5 text-sm font-bold text-gray-900 focus:border-purple-500 focus:outline-none focus:ring-1 focus:ring-purple-500"
              required
            />
            <div className="mt-1 flex justify-between text-[11px] text-gray-500">
              <span>Resulting Balance: {formatCurrency(remainingAfterPayment)}</span>
              <button
                type="button"
                onClick={() => setAmount(formatPaise(outstanding))}
                className="text-purple-600 font-semibold hover:underline"
              >
                Pay Full Balance
              </button>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">
                Payment Method *
              </label>
              <select
                value={paymentMethod}
                onChange={(e) => setPaymentMethod(e.target.value)}
                className="w-full rounded-lg border border-gray-300 p-2.5 text-xs focus:border-purple-500 focus:outline-none"
              >
                <option value="NEFT">NEFT</option>
                <option value="RTGS">RTGS</option>
                <option value="IMPS">IMPS</option>
                <option value="Bank Transfer">Bank Transfer</option>
                <option value="Cheque">Cheque</option>
                <option value="UPI">UPI</option>
                <option value="Cash">Cash</option>
                <option value="Other">Other</option>
              </select>
            </div>

            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">
                Payment Date *
              </label>
              <input
                type="date"
                value={paymentDate}
                onChange={(e) => setPaymentDate(e.target.value)}
                className="w-full rounded-lg border border-gray-300 p-2.5 text-xs focus:border-purple-500 focus:outline-none"
                required
              />
            </div>
          </div>

          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">
              UTR / Reference / Cheque No.
            </label>
            <input
              type="text"
              placeholder="e.g. UTR9988776655"
              value={paymentReference}
              onChange={(e) => setPaymentReference(e.target.value)}
              className="w-full rounded-lg border border-gray-300 p-2.5 text-xs focus:border-purple-500 focus:outline-none focus:ring-1 focus:ring-purple-500 font-mono"
            />
          </div>

          <div>
            <label className="block text-xs font-semibold text-gray-700 mb-1">
              Payment Remarks
            </label>
            <textarea
              rows={2}
              placeholder="e.g. Cleared via HDFC Corporate Account"
              value={paymentRemarks}
              onChange={(e) => setPaymentRemarks(e.target.value)}
              className="w-full rounded-lg border border-gray-300 p-2.5 text-xs focus:border-purple-500 focus:outline-none focus:ring-1 focus:ring-purple-500"
            />
          </div>

          <div className="flex items-center justify-end gap-3 border-t pt-4">
            <button
              type="button"
              onClick={onClose}
              className="rounded-lg border px-4 py-2 text-xs font-medium text-gray-600 hover:bg-gray-100 transition"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="rounded-lg bg-purple-600 px-5 py-2 text-xs font-semibold text-white hover:bg-purple-700 shadow transition disabled:opacity-50"
            >
              {loading ? "Recording..." : "Confirm Payment"}
            </button>
          </div>
        </form>
      </div>
    </div>
  );
};

export default RecordPaymentModal;
