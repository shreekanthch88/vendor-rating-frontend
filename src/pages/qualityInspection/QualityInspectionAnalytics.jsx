...
const QualityInspectionAnalytics = () => {
  ...
  const [data, setData] = useState({
    summary: {
      ...
      trendPercent: 12,
      accepted: { count: 142, rate: 76 },
      conditionallyAccepted: { count: 21, rate: 11 },
      rejected: { count: 23, rate: 13 },
      reInspections: { count: 18, rate: 10 },
      replacementRequests: { count: 12, rate: 6 },
    },
    resultsDistribution: [
    ...
    monthlyTrends: [
      { month: "Apr 2026", acceptanceRate: 76, rejectionRate: 22, defectRate: 11 },
      { month: "May 2026", acceptanceRate: 81, rejectionRate: 18, defectRate: 8 },
      { month: "Jun 2026", acceptanceRate: 79, rejectionRate: 13, defectRate: 7 },
      { month: "Jul 2026", acceptanceRate: 85, rejectionRate: 16, defectRate: 7 },
      { month: "Aug 2026", acceptanceRate: 81, rejectionRate: 15, defectRate: 7 },
      { month: "Sep 2026", acceptanceRate: 88, rejectionRate: 15, defectRate: 7 },
    ],
    topDefects: [
      { reason: "Dimensional Defect", count: 38, color: "#EF4444" },
      { reason: "Surface Damage", count: 26, color: "#3B82F6" },
      { reason: "Material Defect", count: 18, color: "#10B981" },
      { reason: "Wrong Specification", count: 14, color: "#F59E0B" },
      { reason: "Quantity Damage", count: 9, color: "#8B5CF6" },
      { reason: "Other", count: 5, color: "#94A3B8" },
    ],
    vendorPerformance: [
      {
        vendorId: "v1",
      ...
      {
        vendorId: "v4",
        vendorName: "Tech Materials Supply",
        inspections: 22,
        acceptedRate: 78,
        rejectedRate: 15,
        defectRate: 11.2,
  ...
  const reinspectionChartData = useMemo(() => {
    const passed = data.reinspectionAnalysis?.passedAfterCorrection || 13;
    const failed = data.reinspectionAnalysis?.failedAgain || 5;
    return [
  ...
  const replacementChartData = useMemo(() => {
    const app = data.replacementAnalysis?.approved || 9;
    const rej = data.replacementAnalysis?.rejected || 2;
    const pen = data.replacementAnalysis?.pending || 1;
    return [
  ...
  const vendorComparisonData = useMemo(() => {
    ...
    return list.slice(0, 5).map((v, i) => ({
      ...
      value: comparisonMetric === "acceptanceRate" ? v.acceptedRate : v.qualityScore,
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  dataKey="value"
                >
                  {data.resultsDistribution.map((entry, index) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="relative h-44 flex items-center justify-center my-2">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <div className="text-xl font-bold text-slate-900 leading-none">
                {data.reinspectionAnalysis?.total || 18}
              </div>
          ...
          <div className="space-y-2 pt-2 border-t border-slate-100 text-xs">
            <div className="flex items-center justify-between">
              ...
              <span className="font-bold text-slate-900">
                {data.reinspectionAnalysis?.passedAfterCorrection || 13} (
                {data.reinspectionAnalysis?.passedRate || 72}%)
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="flex items-center gap-1.5 text-slate-600">
              ...
              <span className="font-bold text-slate-900">
                {data.reinspectionAnalysis?.failedAgain || 5} (
                {data.reinspectionAnalysis?.failedRate || 28}%)
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 bg-white rounded-2xl border border-slate-200/80 p-5 shadow-xs flex flex-col justify-between">
          ...
          <div className="relative h-44 flex items-center justify-center my-2">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <div className="text-xl font-bold text-slate-900 leading-none">
                {data.replacementAnalysis?.total || 12}
              </div>
          ...
          <div className="space-y-1.5 pt-2 border-t border-slate-100 text-xs">
            <div className="flex items-center justify-between">
              ...
              <span className="font-bold text-slate-900">
                {data.replacementAnalysis?.approved || 9} ({data.replacementAnalysis?.approvedRate || 75}%)
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="flex items-center gap-1.5 text-slate-600">
              ...
              <span className="font-bold text-slate-900">
                {data.replacementAnalysis?.rejected || 2} ({data.replacementAnalysis?.rejectedRate || 17}%)
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="flex items-center gap-1.5 text-slate-600">
              ...
              <span className="font-bold text-slate-900">
                {data.replacementAnalysis?.pending || 1} ({data.replacementAnalysis?.pendingRate || 8}%)
              </span>
            </div>
          </div>
        </div>
